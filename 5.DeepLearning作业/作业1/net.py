...
class mixed_net(nn.Module): 
    ...
    def forward(self, x):
        ...
        '''
        x = self.pool(torch.relu(self.bn1(self.conv1(x)))) # 卷积层1
        x = self.pool(torch.relu(self.bn2(self.conv2(x)))) # 卷积层2
        x = self.pool(torch.relu(self.bn3(self.conv3(x)))) # 卷积层3
        x = self.pool(torch.relu(self.bn4(self.conv4(x)))) # 卷积层4
        
        x = x.reshape(x.size(0), -1) # 展平
        x = self.dropout(torch.relu(self.linear1(x))) # 全连接层1
        x = self.dropout(torch.relu(self.linear2(x))) # 全连接层2  
        x = torch.relu(self.linear3(x)) # 全连接层3
        x = self.output(x) # 输出层
        return x
